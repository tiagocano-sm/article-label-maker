...
def create_classification_prompt(text: str, labels: List[str], max_examples: int = 8) -> str:
    ...
    """
    prompt = "You are a medical text classifier. Classify the following medical article into one or several of these categories(separated by '|'):\n"
    ...
    # prompt += "The article might contain multiple labels separated by '|'. Please classify the article into one of these categories based on the examples:\n"
    prompt += f"{text}\n"
    prompt += "Category:"
